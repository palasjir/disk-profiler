...
export function updateDirInfoUp(
...
export function createSetDirectoryUpdater(
    newDir: DirectoryNode,
    oldDir?: DirectoryNode
): DirInfoUpdater {
    ...
    const dirsDiff =
        oldMeta.totalNumberOfDirectories -
        newDir.totalNumberOfDirectories -
        plusOne

    return {
        sizeInBytes: current => current.sizeInBytes - sizeDiff,
        totalNumberOfFiles: current => current.totalNumberOfFiles - filesDiff,
        totalNumberOfDirectories: current =>
            current.totalNumberOfDirectories - dirsDiff,
    }
}

export function createRemoveDirectoryUpdater(
    dirToRemove: DirectoryNode
): DirInfoUpdater {
    return {
        sizeInBytes: current => current.sizeInBytes - dirToRemove.sizeInBytes,
        totalNumberOfFiles: current =>
            current.totalNumberOfFiles - dirToRemove.totalNumberOfFiles,
        totalNumberOfDirectories: current =>
            current.totalNumberOfDirectories -
            dirToRemove.totalNumberOfDirectories -
            1,
    }
}

export function createRemoveFileUpdater(
...
export function createAddEmptyDirectoryUpdater(): DirInfoUpdater {
    return {
        totalNumberOfDirectories: current =>
            current.totalNumberOfDirectories + 1,
    }
}
